...
__global__ void apply_clipping(unsigned int *histograms, int *nrExcess, unsigned int clipLimit) {
    ...
    //indice del livello
    unsigned int hid = (bid*256) + threadIdx.x;
    ...
    if ( clipLimit > 0 ) {
        ...
        unsigned int binIncr = (int)(nrExcess[bid] / 256);
        ...
        if ( nrExcess[bid] > 0 ) {
            ...
            int stepSz = (int)(nrExcess[bid] / 256) ; 
    ...
    //ogni livello calcola la propria funzione cumulativa
    unsigned int cumulated = 0;
    for (unsigned int i = 0 ; i <= threadIdx.x; i++) {
        cumulated += histograms[(bid*256)+i];
    } 
    
    __syncthreads();

    histograms[hid]=cumulated;                                            
}
...
__global__ void apply_cdfs(unsigned char *image, unsigned char *outputImage, int *cdfs, unsigned int w, unsigned int h) {   
    ...
    int equalizedPixel = (cdfs[(idx*256)+pixelValue] - cdfs[idx*256]) * 255 / ((32 * 32) - cdfs[idx*256]);
...
def histogram_equalization_cuda(input_image):
    ...
    # lancio del kernel clipping e calcolo della cdf
